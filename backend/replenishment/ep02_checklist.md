...
| AC05 Triggers booleans included (activate_B/activate_C/activate_all) | Pass | `backend/replenishment/services/needs_list.py` (`build_preview_items`) | Critical items driven by config (`NEEDS_CRITICAL_ITEM_IDS` / `NEEDS_CRITICAL_CATEGORY_IDS`) until DB model exists. |

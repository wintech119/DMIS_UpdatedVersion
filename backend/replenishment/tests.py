...
def _ensure_legacy_reference_rows() -> None:
    ...
    legacy_table_ddls = [
        ...
        CREATE TABLE IF NOT EXISTS ref_event_phase (
            phase_code VARCHAR(32) PRIMARY KEY,
        ...
        CREATE TABLE IF NOT EXISTS item (
            ...
            default_uom_code VARCHAR(25),
            units_size_vary_flag BOOLEAN,
            usage_desc TEXT,
            storage_desc TEXT,
            is_batched_flag BOOLEAN,
            can_expire_flag BOOLEAN,
            issuance_order VARCHAR(20),
            comments_text TEXT,
            status_code VARCHAR(1),
            create_by_id VARCHAR(20),
            create_dtime TIMESTAMP,
...
class GlobalPhaseWindowPolicyTests(TestCase):
    @override_settings(
    ...
    def test_set_global_phase_windows_persists_uppercase_json_config_type(self) -> None:
        ...
        insert_calls = [
            ...
            if call.args and "INSERT INTO tenant_config" in str(call.args[0])
        ]
...
class NeedsListPreviewApiTests(TestCase):
    ...
    @patch("replenishment.views.data_access.get_item_names")
    ...
    def test_preview_endpoint_returns_response_with_missing_burn_warning(
        self,
        mock_available,
        mock_donations,
        mock_transfers,
        ...
        mock_burn.return_value = ({}, ["burn_data_missing"], "reliefpkg", {"filter": burn_filter})
        ...
        self.assertIn("burn_data_missing", body["warnings"])
        self.assertIn("debug_summary", body)
        self.assertEqual(body["debug_summary"]["burn"].get("filter"), burn_filter)
...
class NeedsListWorkflowApiTests(TestCase):
    ...
    def test_odpem_replenishment_needs_list_can_still_generate_transfer_sourcing(self) -> None:
        record = {
        ...
        with patch("replenishment.views._require_record_scope", return_value=None), patch(
            "replenishment.views.workflow_store.store_enabled_or_raise"
        ), patch(
            "replenishment.views.workflow_store.get_record", return_value=record
        ), patch(
            "replenishment.views.data_access.get_warehouses_with_stock",
    ...
    def test_donations_export_post_queues_and_deduplicates_async_jobs(
        ...
        ), patch("replenishment.views.workflow_store.store_enabled_or_raise"), patch(
            "replenishment.views.workflow_store.get_record", return_value=record
        ):
    ...
    def test_donations_allocate_requires_db_backed_execution_context(self) -> None:
        record = {
            "needs_list_id": "NL-A",
            "status": "APPROVED",
            "snapshot": {"items": []},
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            with self.settings(DEV_AUTH_ROLES=["EXECUTIVE"]):
                approve_same_user = self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-same-user-{needs_list_id}",
                )
            ...
            with self.settings(
                ...
                approve = self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-reviewer-{needs_list_id}",
                )
    ...
    def test_request_changes_requires_reason_code_and_allows_resubmit(
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            self.assertEqual(returned_ok.json().get("status"), "RETURNED")
            ...
            with self.settings(
                ...
                self.assertEqual(resubmit.json().get("status"), "PENDING_APPROVAL")
    ...
    def test_owner_scoped_visibility_ignores_updated_by_for_modified_records(
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            self.assertEqual(returned.json().get("status"), "RETURNED")
    ...
    def test_fulfillment_sources_use_overridden_required_qty_for_tracker_math(
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            record = workflow_store.get_record(needs_list_id)
            self.assertIsNotNone(record)
            needs_list_item = NeedsListItem.objects.get(
                needs_list_id=int(needs_list_id),
                item_id=1,
            )
            needs_list_item.fulfilled_qty = Decimal("3.00")
            needs_list_item.fulfillment_status = "PARTIAL"
            needs_list_item.save(update_fields=["fulfilled_qty", "fulfillment_status"])
            ...
            needs_list_item.fulfilled_qty = Decimal("5.00")
            needs_list_item.fulfillment_status = "FULFILLED"
            needs_list_item.save(update_fields=["fulfilled_qty", "fulfillment_status"])
    ...
    def test_create_draft_does_not_supersede_when_primary_warehouse_ids_differ(
        self,
    ) -> None:
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            first = workflow_store.create_draft(
    ...
    def test_rbac_denies_unauthorized_approve(
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            approve = self.client.post(
                ...
                {},
                format="json",
                HTTP_IDEMPOTENCY_KEY=f"approve-blocked-{needs_list_id}",
            )
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            with self.settings(DEV_AUTH_ROLES=["EXECUTIVE"], DEV_AUTH_USER_ID="reviewer"):
                approve = self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-executive-{needs_list_id}",
                )
                self.assertEqual(approve.status_code, 200)
                body = approve.json()
            ...
            with self.settings(DEV_AUTH_ROLES=["LOGISTICS"], DEV_AUTH_USER_ID="approver"):
                approve_denied = self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-denied-{needs_list_id}",
                )
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            with self.settings(DEV_AUTH_ROLES=["LOGISTICS"], DEV_AUTH_USER_ID="approver"):
                approve = self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-donation-{needs_list_id}",
                )
                self.assertEqual(approve.status_code, 200)
                self.assertEqual(approve.json().get("status"), "APPROVED")
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            with self.settings(DEV_AUTH_ROLES=["LOGISTICS"], DEV_AUTH_USER_ID="approver"):
                approve = self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-procurement-denied-{needs_list_id}",
                )
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            with self.settings(DEV_AUTH_ROLES=["EXECUTIVE"], DEV_AUTH_USER_ID="submitter"):
                approve = self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-submitter-{needs_list_id}",
                )
            self.assertEqual(approve.status_code, 409)
    ...
    def test_escalate_requires_reason(
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            self.assertEqual(escalated.status_code, 200)
            self.assertEqual(escalated.json().get("status"), "UNDER_REVIEW")
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            with self.settings(DEV_AUTH_ROLES=["EXECUTIVE"], DEV_AUTH_USER_ID="reviewer"):
                approve = self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-dispatch-flow-{needs_list_id}",
                )
                self.assertEqual(approve.status_code, 200)
        ...
        with patch(
            ...
            with self.settings(DEV_AUTH_ROLES=["EXECUTIVE"], DEV_AUTH_USER_ID="reviewer"):
                approve = self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-receipt-flow-{needs_list_id}",
                )
                self.assertEqual(approve.status_code, 200)
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            with self.settings(DEV_AUTH_ROLES=["EXECUTIVE"], DEV_AUTH_USER_ID="reviewer"):
                self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-execution-denied-{needs_list_id}",
                )
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            with self.settings(DEV_AUTH_ROLES=["EXECUTIVE"], DEV_AUTH_USER_ID="reviewer"):
                self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-cancel-{needs_list_id}",
                )
            ...
            with self.settings(DEV_AUTH_ROLES=["EXECUTIVE"], DEV_AUTH_USER_ID="reviewer"):
                self.client.post(
                    ...
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-cancel-dispatch-{needs_list_id_two}",
                )
    ...
    def test_approval_requires_escalation_for_cross_parish_over_500(
        ...
        with patch.dict(os.environ, {"NEEDS_WORKFLOW_DEV_STORE": "1"}):
            ...
            escalation_summary = {
                "total_required_qty": 600.0,
                "total_estimated_cost": None,
                "approval": {"tier": "Tier 3", "approver_role": "Director General"},
                "rationale": "Cross-parish transfer over 500 units requires escalation.",
                "warnings": ["transfer_cross_parish_over_500"],
                "escalation_required": True,
            }
            with self.settings(DEV_AUTH_ROLES=["EXECUTIVE"], DEV_AUTH_USER_ID="reviewer"), patch(
                "replenishment.views._approval_summary_for_record",
                return_value=escalation_summary,
            ):
                approve = self.client.post(
                    f"/api/v1/replenishment/needs-list/{needs_list_id}/approve",
                    {},
                    format="json",
                    HTTP_IDEMPOTENCY_KEY=f"approve-escalation-{needs_list_id}",
                )
                self.assertEqual(approve.status_code, 409)

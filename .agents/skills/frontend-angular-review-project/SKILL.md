...
- Do not approve a change that duplicates or drifts the design tokens defined in `frontend/src/lib/prompts/generation.ts` (warm-neutral palette, status tones, typography, spacing, radius). Visual values must come from that file via `frontend/src/styles.scss`.

...
def get_lookup_tables_info(conn) -> dict:
    ...
    if item_tables:
        ...
        if 'name' in cols:
            ...
            info['items'] = {row[1]: row[0] for row in cur.fetchall()}
        elif 'item_name' in cols:
            cur.execute(f"SELECT id, item_name FROM {item_table}")
            info['items'] = {row[1]: row[0] for row in cur.fetchall()}
        elif 'description' in cols:
            cur.execute(f"SELECT id, description FROM {item_table}")
            info['items'] = {row[1]: row[0] for row in cur.fetchall()}
    
    # Check for warehouse table
    cur.execute("""
        SELECT table_name 
        FROM information_schema.tables 
        WHERE table_schema = 'public' 
    ...
    if wh_tables:
        ...
        if 'code' in cols:
            ...
            info['warehouses'] = {row[1]: row[0] for row in cur.fetchall()}
        elif 'name' in cols:
...
def migrate_records(conn, records: list[dict], lookup_info: dict,
    ...
    for r in records:
        ...
        if not item_id and lookup_info['has_item_table']:
            # Item not found in lookup - might need to create it

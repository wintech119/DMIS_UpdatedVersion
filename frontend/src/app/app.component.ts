...
const ROUTE_BREADCRUMBS: { pattern: RegExp; crumbs: (match: RegExpMatchArray) => BreadcrumbSegment[] }[] = [
  ...
  { pattern: /^\/replenishment\/needs-list\/(.+)\/allocation$/, crumbs: (m) => [{ label: 'Supply Replenishment', route: '/replenishment/dashboard' }, { label: 'Allocation Workspace' }, { label: `List #${m[1]}` }] },
  { pattern: /^\/replenishment\/needs-list\/(.+)\/dispatch$/, crumbs: (m) => [{ label: 'Supply Replenishment', route: '/replenishment/dashboard' }, { label: 'Dispatch Workspace' }, { label: `List #${m[1]}` }] },

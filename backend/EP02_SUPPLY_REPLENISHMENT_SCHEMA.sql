...
-- ----------------------------------------------------------------------------
-- 2.5A NEW: needs_list_donation_draft_line - Horizon B draft allocations
-- ----------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS public.needs_list_donation_draft_line (
    donation_draft_line_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    needs_list_id INTEGER NOT NULL REFERENCES public.needs_list(needs_list_id) ON DELETE CASCADE,
    needs_list_item_id INTEGER REFERENCES public.needs_list_item(needs_list_item_id) ON DELETE SET NULL,
    item_id INTEGER NOT NULL REFERENCES public.item(item_id),
    uom_code VARCHAR(25) NOT NULL,
    required_qty NUMERIC(15,4) NOT NULL DEFAULT 0.0000,
    allocated_qty NUMERIC(15,4) NOT NULL DEFAULT 0.0000,
    inventory_id INTEGER REFERENCES public.inventory(inventory_id),
    batch_id INTEGER REFERENCES public.item_batch(batch_id),
    ...
    CONSTRAINT c_nlddl_qty CHECK (required_qty >= 0.0000 AND allocated_qty >= 0.0000),
    CONSTRAINT c_nlddl_status CHECK (status_code IN ('DRAFT', 'SUPERSEDED'))
);

CREATE INDEX idx_nlddl_needs_list_status ON public.needs_list_donation_draft_line(needs_list_id, status_code);
CREATE INDEX idx_nlddl_item_status ON public.needs_list_donation_draft_line(item_id, status_code);
CREATE INDEX idx_nlddl_source ON public.needs_list_donation_draft_line(source_record_id);

COMMENT ON TABLE public.needs_list_donation_draft_line IS 'Durable Horizon B donation draft lines generated on needs list approval without executing allocation';
...
6. needs_list_donation_draft_line - Horizon B donation draft allocations
7. burn_rate_snapshot - Historical burn rate calculations
8. warehouse_sync_log - Data freshness tracking
9. procurement - Horizon C procurement orders
10. procurement_item - Procurement line items
11. supplier - Supplier/vendor master
12. lead_time_config - Configurable lead times

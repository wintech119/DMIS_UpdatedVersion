...
- When adding idempotency to multi-tenant Django write paths, scope the cache key by actor plus tenant/resource and reserve the key before the write path so concurrent retries cannot bypass access checks or race each other past the first cache miss.

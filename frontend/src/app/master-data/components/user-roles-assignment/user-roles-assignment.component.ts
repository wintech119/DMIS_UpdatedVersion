...
@Component({
  selector: 'dmis-user-roles-assignment',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './user-roles-assignment.component.html',
  styleUrl: './user-roles-assignment.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class UserRolesAssignmentComponent implements OnInit {
  ...
  loadAssignments(): void {
    ...
    this.assignmentService.listUserRoles(this.userId()).pipe(
      takeUntilDestroyed(this.destroyRef),
    ).subscribe({
      next: (roles) => {
      ...
      error: (error: unknown) => {
        this.isLoading.set(false);
        this.errorMessage.set(this.getErrorMessage(error, 'Unable to load assigned roles.'));
      },
    });
  }
  ...
  assignSelectedRole(): void {
    ...
    this.assignmentService.assignUserRole(this.userId(), role.role_id).pipe(
      takeUntilDestroyed(this.destroyRef),
    ).subscribe({
      next: () => {
      ...
      error: (error: unknown) => {
        this.isSaving.set(false);
        this.showToastIfClientError(error, 'Unable to assign role.');
      },
    });
  }
  ...
  confirmRevoke(role: UserRoleAssignment): void {
    ...
    const dialogRef = this.dialog.open(DmisConfirmDialogComponent, {
      data: {
        title: 'Remove role',
        message: `Remove role ${role.code} from this user? They will lose any permissions granted only by this role.`,
  ...
  private revokeRole(roleId: number): void {
    this.isSaving.set(true);
    this.assignmentService.revokeUserRole(this.userId(), roleId).pipe(
      takeUntilDestroyed(this.destroyRef),
    ).subscribe({
      next: () => {
        this.isSaving.set(false);
      ...
      error: (error: unknown) => {
        this.isSaving.set(false);
        this.showToastIfClientError(error, 'Unable to remove role.');
      },
    });
  }

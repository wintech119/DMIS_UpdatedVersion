...
2. **Verify the architecture-review gate ran**. If a medium- or high-risk change has not run `system-architecture-review`, mark `run architecture-review` as a Required Change and refuse to approve.
...
4. **Verify reuse**. Anything that should have used `_parse_*` from `replenishment/views.py:315-373` or `validate_record` from `masterdata/services/validation.py:20` and didn't is a finding.

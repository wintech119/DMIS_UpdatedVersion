...
class ApplyReplenishmentSqlMigrationCommandTests(SimpleTestCase):
    ...
    def test_apply_executes_rendered_sql_for_selected_template(self) -> None:
        ...
        self.assertIn("AS inbound_start_dtime", executed_sql)
        self.assertIn("AS inbound_end_dtime", executed_sql)
        self.assertIn("COALESCE(t.dispatched_at, t.update_dtime, t.create_dtime)", executed_sql)
        self.assertIn("COALESCE(d.verify_dtime, d.update_dtime, d.create_dtime)", executed_sql)
        self.assertIn("COALESCE(p.shipped_at, p.update_dtime, p.create_dtime)", executed_sql)

...
const ROUTE_BREADCRUMBS: { pattern: RegExp; crumbs: (match: RegExpMatchArray) => BreadcrumbSegment[] }[] = [
  ...
  { pattern: /^\/replenishment\/inventory\/repackaging$/, crumbs: () => [{ label: 'Inventory', route: '/replenishment/inventory/repackaging' }, { label: 'UOM Repackaging' }] },
  { pattern: /^\/replenishment\/inventory\/repackaging\/(.+)$/, crumbs: (m) => [{ label: 'Inventory', route: '/replenishment/inventory/repackaging' }, { label: 'UOM Repackaging', route: '/replenishment/inventory/repackaging' }, { label: `Transaction #${m[1]}` }] },

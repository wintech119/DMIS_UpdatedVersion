...
- `frontend/src/lib/prompts/generation.tsx` - Frontend visual and component-pattern source of truth
...
- frontend route structure, lazy module boundaries, shared component reuse, or design-system tokens (`generation.tsx`)
...
- comment-only edits
- isolated styling changes with no architecture, behavior, or security impact (and that conform to `frontend/src/lib/prompts/generation.tsx`)
...
- **PreToolUse on Write/Edit** - Block direct edits to `docs/adr/system_application_architecture.md`, `docs/security/**`, and `frontend/src/lib/prompts/generation.tsx` without an explicit confirmation prompt
...
- Hard-coding visual values that drift from `generation.tsx` tokens
